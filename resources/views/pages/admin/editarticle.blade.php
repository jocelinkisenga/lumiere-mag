...
<style>
    ...
    .preview-container { margin-top: 15px; position: relative; }
        ...
        <div class="col-lg-10 col-md-12">
            <div class="card">
                <div class="card-body p-4">
                    ...
                    <form id="articleForm" method="POST" action="{{ route('posts.update', $post->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="col-md-6 mb-3" id="tour-author">
                                    ...
                                    <select class="form-control" name="author_id" id="author_id">
                                        ...
                                        @foreach (\App\Models\Author::get() as $item)
                                            <option value="{{ $item->id }}" {{ (old('author_id', $post->author_id) == $item->id) ? 'selected' : '' }}>
                                                {{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="form-step" id="step-2">
                            <div id="tour-editor-container">
                                <label class="form-label mb-3">Modifiez votre article ci-dessous</label>
                                <textarea name="description" id="edit">{!! old('description', $post->description) !!}</textarea>
                            </div>
                            
                            <div class="d-flex justify-content-between mt-4">
                                <button type="button" class="btn btn-light btn-lg btn-mobile" onclick="goToStep(1)">← Retour</button>
                                <button type="submit" id="btn-publish" class="btn btn-warning btn-lg btn-mobile" style="color:#fff; border:none;">✏️ Mettre à jour</button>
                            </div>
                        </div>
...
<script>
    ...
    document.getElementById('image-input').addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            // Remet l'image d'origine si l'utilisateur annule le choix de fichier
            previewImage.src = "{{ asset('storage/' . $post->image) }}";
            previewBox.style.display = 'block';
        }
    });
    ...
    window.addEventListener('load', function() {
        ...
        const driverObj = driver({
            ...
            doneBtnText: 'Compris !',
            steps: [
                { element: '#tour-title', popover: { title: 'Titre de l\'article', description: 'Modifiez le titre de votre sujet.', position: 'bottom' } },
                { element: '#tour-author', popover: { title: 'L\'Auteur', description: 'Changez l\'auteur si nécessaire.', position: 'bottom' } },
                { element: '#tour-image', popover: { title: 'L\'Image', description: 'Uploadez une nouvelle image uniquement si vous voulez remplacer l\'actuelle.', position: 'bottom' } },
                { element: '#btn-next-step', popover: { title: 'Passer à la rédaction', description: 'Passez à l\'étape suivante.', position: 'top' },
                    onDeselected: () => {
                        if(document.getElementById('step-1').classList.contains('active')) goToStep(2);
                    }
                },
                { element: '#btn-publish', popover: { title: 'Mise à jour ✏️', description: 'Enregistrez vos modifications.', position: 'top' },
        ...
        if (!localStorage.getItem('onboarding_edit_post')) {
            driverObj.drive();
            localStorage.setItem('onboarding_edit_post', 'true');
        }
    });

    // Validation CKEditor avant envoi
